                ...
                {loadingGenerations ? (
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                        {projectGenerations.map(gen => (
                            ...
                            >
                                <img src={gen.thumbnailUrl || historyService.getOptimizedUrl(gen.url, 400)} alt={gen.prompt} loading="lazy" className="w-full h-full object-cover aspect-square bg-slate-900"
                                    onError={(e) => {
                                        const target = e.target as HTMLImageElement;
                                        if (target.src !== gen.url) {
                                            target.src = gen.url;
                                        }
                                    }}
                                />
